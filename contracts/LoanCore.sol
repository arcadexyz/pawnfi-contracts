...
contract LoanCore is ILoanCore {
    ...
    /**
     * @dev Clean up a loan once it is in its terminal state (Repaid or Defaulted)
     * Frees unnecessary state and burns the loan notes
     */
    function cleanupLoan(uint256 loanId, LoanData memory data) internal {
        lenderNote.burn(data.lenderNoteId);
        borrowerNote.burn(data.borrowerNoteId);

        // TODO: open question - these lines save quite a bit of gas
        // but mean that we lose some data on-chain when loans are terminated - worth it?
        // we can always recover the state by reading events, so nothing is really lost
        delete loans[loanId];
        delete collateralInUse[data.terms.collateralTokenId];
    }
