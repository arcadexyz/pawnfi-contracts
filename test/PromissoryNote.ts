...
describe("PromissoryNote", () => {
  ...
  const setupTestContext = async (): Promise<TestContext> => {
    ...
    const lenderPromissoryNote = <PromissoryNote>await deploy("PromissoryNote", signers[0], [loanCore.address, "PromissoryNote - Lender", "PBL"]);
    const borrowerPromissoryNote = <PromissoryNote>await deploy("PromissoryNote", signers[0], [loanCore.address, "PromissoryNote - Borrower", "PBNs"]);
  ...
  describe("burn", () => {
    it("Reverts if loanCore attempts to burn active note", async () => {
      const { borrowerPromissoryNote: promissoryNote, lenderPromissoryNote, loanCore, mockAssetWrapper, user } = await setupTestContext();
    ...
    it("Reverts if sender does not own the note", async () => {
      const { borrowerPromissoryNote: promissoryNote, lenderPromissoryNote, loanCore, mockAssetWrapper, user, other } = await setupTestContext();
      const loanTerms = createLoanTerms(mockAssetWrapper.address);
      const PromissoryNoteId = await mintPromissoryNote(promissoryNote, user);
      const loanId = await createLoan(loanCore, user, loanTerms);
      await startLoan(loanCore, user, lenderPromissoryNote, promissoryNote, loanId);
